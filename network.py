...
class DeepNetwork:
    ...
    def accuracy(self, y_true, y_pred):
        ...
        '''
        acc = len(tf.where(y_true == y_pred))/y_true.shape[0]
        return acc
    ...
    def predict(self, x, output_layer_net_act=None):
        ...
        y_pred = tf.math.argmax(output_layer_net_act, axis = 1)
    ...
    def loss(self, out_net_act, y, eps=1e-16):
        ...
        if self.loss_name == "cross_entropy":
            correct_net_acts = arange_index(out_net_act, y)
            loss = -tf.reduce_mean(tf.math.log(correct_net_acts + eps))

        elif self.loss_name == "lp":
            one_hot_y = tf.one_hot(y, depth=out_net_act.shape[1], on_value=1.0, off_value=-1.0)
        
            # print("net act: ", out_net_act)
            # print("one-hot", one_hot_y)
            # print("abs: ", tf.math.abs(one_hot_y - out_net_act))
            # print("abs_square: ", tf.math.abs(one_hot_y - out_net_act)**2
            loss = tf.reduce_mean(tf.math.abs(one_hot_y - out_net_act)**self.loss_exp)
    ...
    def train_step(self, x_batch, y_batch):
        ...
        with tf.GradientTape() as tape:
            ...
            #TODO: ask the prof whether we wanna average the loss across minibatches
            loss = self.loss(out_net_act=net_act, y=y_batch)
    ...
    def test_step(self, x_batch, y_batch):
        ...
        '''
        net_act = self(x_batch)
        y_pred = self.predict(x_batch, output_layer_net_act=net_act)
        loss = self.loss(out_net_act=net_act, y = y_batch)
        accuracy = self.accuracy(y_true = y_batch, y_pred = y_pred)
    ...
    def fit(self, x, y, x_val=None, y_val=None, batch_size=128, max_epochs=10000, val_every=1, print_every=10,
        ...
        val_loss_early = [] # for early stopping
        val_loss_lr_decay = [] # for lr decay 
        ...
        #TODO: tmrw we ask prof. about the seed
        r_seed = 0
        rng = np.random.default_rng(r_seed)
        if verbose > 0:
            print(f"Starting to train network...There will be {max_epochs} epochs and {num_batch*max_epochs} iterations total,  {num_batch} iter/epoch.")
        ...
        for epoch in range(max_epochs):
            ...
            for i in range(num_batch):
                ...
                batch_indices = rng.choice(num_samples, batch_size, replace=True)
                      
                mini_batch_data = tf.gather(x, batch_indices)
                mini_batch_int_labels = tf.gather(y, batch_indices)

                loss = self.train_step(mini_batch_data, mini_batch_int_labels)

                mini_loss += loss
            
            train_loss = float(mini_loss / num_batch)
            ...
            if epoch % val_every == 0:
                ...
                val_loss_early, stop = self.early_stopping(val_loss_early, val_loss, patience=patience)
                val_loss_lr_decay, lr_update = self.early_stopping(val_loss_lr_decay, val_loss, patience=lr_patience)
                
                self.set_layer_training_mode(is_training=True) # bring back to the training mode
                print(f"Completed Epoch {epoch}/{max_epochs}. Training loss: {train_loss}. Validation Loss: {val_loss}. Validation acc: {val_acc}.")
         
                val_acc_hist.append(val_acc)
                val_loss_hist.append(val_loss)

                if lr_update and lr_decay_count < lr_max_decays:
                    self.lr_step_decay(lr_decay_rate=lr_decay_factor)
                    val_loss_lr_decay = []
                    lr_decay_count += 1
                    print(f"Learning rate decay count: {lr_decay_count}")
                if stop == True:
                    max_epochs = epoch
                    break
            ...
            #         print(f"Completed Epoch {epoch}/{max_epochs}. Training loss: {loss}. Validation Loss: {val_loss}. Validation acc: {val_acc}.")
    ...
    def early_stopping(self, recent_val_losses, curr_val_loss, patience):
        ...
        # when early stopping is eligible
        if len(recent_val_losses) == (patience + 1):
    ...
    def lr_step_decay(self, lr_decay_rate):
        ...
        '''
        print("-"*10)
        print("Optimizer Learning Rate Before: ", float(self.opt.learning_rate))
        self.opt.learning_rate.assign(lr_decay_rate * self.opt.learning_rate)
        print("Optimizer Learning Rate After: ", float(self.opt.learning_rate))
        print("-"*10)