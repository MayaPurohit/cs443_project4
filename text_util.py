...
def get_most_similar_words(k, word_str, all_embeddings, word_str2int, eps=1e-10):
    ...
    -
    '''
    word_ind = word_str2int[word_str]
    word_vec = all_embeddings[word_ind]


    dot_products = tf.linalg.matvec(all_embeddings, word_vec) 

    all_norms = tf.norm(all_embeddings, axis=1) 
    word_norm = tf.norm(word_vec)  

    cos_sim = dot_products / (all_norms * word_norm + eps)  

           
    top_k_sims, top_k_inds = tf.math.top_k(cos_sim, k = k+1, sorted = True)
    result = tf.math.top_k(tf.transpose(cos_sim), k = k+1, sorted = True)
    
    top_k_sims = result.values.numpy()
    top_k_inds = result.indices.numpy()

    
    return top_k_inds, top_k_sims
