...
class Layer:
    ...
    def set_activation_function(self, act_fun_str):
        '''Sets the activation functipn to the string parameter'''
    ...
    def compute_net_activation(self, net_in):
        ...
        if self.act_fun_name == 'relu':
            ...
            net_act = tf.nn.tanh(net_in * self.tanh_beta)

        elif self.act_fun_name == "leaky_relu":
            alpha = self.alpha
            net_act = tf.nn.leaky_relu(net_in, alpha=alpha)
    ...
    def init_groupnorm_params(self):
        ...
        self.b = tf.Variable(tf.zeros(shape=(self.units,)), trainable=False)
...
class Dense(Layer):
    ...
    def init_params(self, input_shape):
        ...
        if self.wt_init == "normal":
            ...
            wt_k = self.get_kaiming_gain()
            stddev = tf.math.sqrt(wt_k/M)


        self.wts = tf.Variable(tf.random.normal(shape=(M, self.units), mean=0, stddev=stddev), trainable=True)
    ...
    def compute_group_norm(self, net_in, eps=0.001):
        ...
        num_neurons_per_group = H // self.num_groups

        net_in_grouped = tf.reshape(net_in, shape = (B, self.num_groups, num_neurons_per_group))
        net_in_2 = (net_in_grouped - tf.math.reduce_mean(net_in_grouped, axis = 2, keepdims = True))/ (tf.math.reduce_std(net_in_grouped, axis = 2, keepdims = True) + eps) 
        net_in_3 = tf.reshape(net_in_2, shape = net_in.shape)
        net_in = self.gn_gain * net_in_3 + self.gn_bias

        return tf.cast(net_in, dtype=tf.float32)
...
class Flatten(Layer):
    ...
    def compute_net_input(self, x):
        ...
        '''
        net_in = tf.reshape(x, shape=(x.shape[0], tf.math.reduce_prod(x.shape[1:])))
        return net_in
